            ...
            bool useClientKeys)
        {
            ...
            for (int ii = 0; ii < signature.Length; ii++)
            {
                if (computedSignature[ii] != signature[ii])
                {
                    ...
                    message.Append("Could not validate signature.");
                    message.AppendLine();
                    message.AppendFormat("ChannelId ={0}, TokenId ={1}, MessageType ={2}, Length ={3}",
                        token.ChannelId,
                        token.TokenId,
                        messageType,
                        messageLength
                    ...
                    message.AppendLine();
                    message.AppendFormat("ExpectedSignature={0}", expectedSignature);
                    message.AppendLine();
                    message.AppendFormat("ActualSignature={0}", actualSignature);
                    return false;
                }
            }
